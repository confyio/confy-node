...
confy.config.match = function (url) {
  ...
  if (url.host && url.user && url.pass && url.heroku) {
    url.path = '/heroku/config';
  } else if (url.host && url.token && url.org) {
    url.path = '/orgs/' + url.org + '/config/' + url.token;
  } else if (url.host && url.user && url.pass && url.org && url.project && url.env) {
